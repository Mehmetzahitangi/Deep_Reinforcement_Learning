...
if __name__ == "__main__":
    ...
    proj_hist = dqn_extra.distr_projection(src_hist, np.array([2], dtype=np.float32), np.array([False]), gamma=0.9)
    ...
    src_hist = hist[0]
    proj_hist = dqn_extra.distr_projection(np.array([src_hist]), np.array([2], dtype=np.float32), np.array([False]),gamma=0.9)
    save_distr(hist[0], proj_hist[0], "normal-r=2")

    # normal distribution, but done episode
    proj_hist = dqn_extra.distr_projection(np.array([src_hist]), np.array([2], dtype=np.float32), np.array([True]),gamma=0.9)
    save_distr(hist[0], proj_hist[0], "normal-done-r=2")

    # clipping for out-of-range distribution
    proj_dist = dqn_extra.distr_projection(np.array([src_hist]), np.array([10], dtype=np.float32), np.array([False]),gamma=0.9)
    save_distr(hist[0], proj_dist[0], "normal-r=10")

    # test both done and not done, unclipped
    proj_hist = dqn_extra.distr_projection(np.array([src_hist, src_hist]), np.array([2, 2], dtype=np.float32), np.array([False, True]), gamma=0.9)
    ...
    proj_hist = dqn_extra.distr_projection(np.array([src_hist, src_hist]), np.array([10, 10], dtype=np.float32), np.array([False, True]), gamma=0.9)
