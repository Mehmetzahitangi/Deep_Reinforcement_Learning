...
def calc_loss_rainbow(batch, batch_weights, net, tgt_net, gamma, device="cpu", double=True):
    """
    N-Step DQN
    PER (Öncelikli Hafıza)
    Double DQN yapıları burada
    """
...
if __name__ == "__main__":
    ...
    net = dqn_extra.RainbowDQN(env.observation_space.shape, env.action_space.n).to(device) # Dueling Erişimi

    tgt_net = ptan.agent.TargetNet(net)
    selector = ptan.actions.ArgmaxActionSelector() # Noisy Nets
    agent = ptan.agent.DQNAgent(net, selector, device=device)

    exp_source = ptan.experience.ExperienceSourceFirstLast(env, agent, gamma=params.gamma, steps_count=N_STEPS) # Ajan N_STEPS adım sonrasına bakıyor
    buffer = dqn_extra.PrioReplayBuffer(exp_source, params.replay_size, PRIO_REPLAY_ALPHA) # Prioritized Experience Replay
